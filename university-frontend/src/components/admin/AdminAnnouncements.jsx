        ...
        <div>
            ...
            <div style={styles.pageHeader}>
                <div>
                    ...
                    <p style={styles.pageSubtitle}>Create and manage system announcements</p>
                </div>
            ...
            {announcements.length === 0 ? (
                <div style={styles.emptyState}>
                    ...
                    <p style={styles.emptySubtext}>Create your first announcement!</p>
                </div>
            ) : (
                            ...
                            <div style={styles.cardTop}>
                                <div style={styles.cardLeft}>
                                    <div style={styles.titleRow}>
                                        ...
                                        <span style={{ ...styles.priorityBadge, ...getPriorityStyle(a.priority) }}>
                                            {a.priority}
                                        </span>
                                        <span style={styles.targetBadge}>
                                            {a.targetAudience === "all" ? "All Users" : a.targetAudience === "specific" ? "Specific" : "Individual"}
                                        </span>
                                    </div>
                                ...
                                <button onClick={() => setDeleteConfirm(a)} style={styles.deleteBtn}>Delete</button>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Create New Announcement" size="lg">
                <div>
                    <div style={styles.formGroup}>
                        <label style={styles.label}>Title *</label>
                        <input placeholder="Announcement title" value={form.title} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))} style={styles.input} />
                    </div>
                    <div style={styles.formGroup}>
                        <label style={styles.label}>Content *</label>
                        <textarea placeholder="Write your announcement here..." value={form.content} onChange={(e) => setForm((p) => ({ ...p, content: e.target.value }))} rows={4} style={{ ...styles.input, resize: "none" }} />
                    </div>
                    <div style={styles.twoCol}>
                        <div style={styles.formGroup}>
                            <label style={styles.label}>Priority</label>
                            <select value={form.priority} onChange={(e) => setForm((p) => ({ ...p, priority: e.target.value }))} style={styles.input}>
                        ...
                        <div style={styles.formGroup}>
                            ...
                            <select value={form.targetAudience} onChange={(e) => setForm((p) => ({ ...p, targetAudience: e.target.value, targetIds: [] }))} style={styles.input}>
                    ...
                    {form.targetAudience !== "all" && (
                        <div style={styles.formGroup}>
                            ...
                            <select multiple value={form.targetIds} onChange={(e) => { const selected = Array.from(e.target.selectedOptions, (opt) => opt.value); setForm((p) => ({ ...p, targetIds: selected })); }} style={{ ...styles.input, height: "120px" }}>
                                {users.map((u) => (
                                    <option key={u._id} value={u._id}>{u.name} ({u.role}) — {u.email}</option>
                                ))}
                            </select>
                            <p style={styles.hint}>Hold Ctrl to select multiple users.</p>
                        </div>
                    )}
                    <div style={styles.modalActions}>
                        <button onClick={() => setShowModal(false)} style={styles.cancelBtn}>Cancel</button>
                        <button onClick={handleSubmit} style={styles.submitBtn}>Publish Announcement</button>
                    </div>
                </div>
            </Modal>
            ...
            <Modal isOpen={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} title="Confirm Delete" size="sm">
                <div>
                    <p style={styles.confirmText}>Are you sure you want to delete <strong>{deleteConfirm?.title}</strong>?</p>
                    <div style={styles.modalActions}>
                        <button onClick={() => setDeleteConfirm(null)} style={styles.cancelBtn}>Cancel</button>
                        <button onClick={() => handleDelete(deleteConfirm._id)} style={styles.deleteBtnModal}>Delete</button>
                    </div>
                </div>
            </Modal>
        </div>
...
const styles = {
    ...
    targetBadge: { background: "#f1f5f9", color: "#475569", padding: "2px 8px", borderRadius: "99px", fontSize: "11px", fontWeight: "500" },
